			...
			function next() {
				if (!hijacking) { // Start the hijack now.
					...
					redis.set((redisDownloadKey + downloadID), "active", "EX", redisDownloadTTL);
			  		...
			  		var downloadCMD = ffmpegPath + ' -i $(' + ORIGIN_PATH + 'youtube-dl -f bestvideo[ext!=webm]‌​+bestaudio[ext!=webm]‌​/best[ext!=webm] -g ' + twitchStream + ') -c copy -preset medium ' + fileName + '.mp4';
